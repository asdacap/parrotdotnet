...
internal sealed class CompactorAndContextTests : IDisposable
{
    ...
    public async Task Scope_context_renders_only_effective_changes_from_ancestors_to_self()
    {
        ...
        var unscoped = AgentIdentity.Child(
            "unscoped",
            AgentIdentity.Main("main", "main-agent", TestModels.PromptTemplates),
            "unscoped",
            1,
        ...
        var worker = AgentIdentity.Child(
            "worker",
            AgentIdentity.Main("planner", "planner", TestModels.PromptTemplates),
            "worker",
            2,
            inheritedScope,
            TestModels.PromptTemplates).Context;
        var implementer = AgentIdentity.Child(
            "implementer",
            AgentIdentity.Main("reviewer", "reviewer", TestModels.PromptTemplates),
            "implementer",
            4,
            changedScope,
            TestModels.PromptTemplates).Context;
        ...
        CancellationToken cancellationToken)
    {
        ...
        _ = await session.Send(
            [ConversationPart.TextPart("third")], Identifier.MessageId(), Delivery.Steer, new IncomingActivity(string.Empty, null), cancellationToken);
        await provider.Arrived(cancellationToken);
