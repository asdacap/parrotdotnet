...
internal sealed class ToolDefinitionTests : IDisposable
{
    ...
    public async Task Every_builtin_tool_has_a_complete_configured_definition()
    {
        ...
        var expected = new[]
        {
            "agent_send", "agent_spawn", "agent_status", "edit", "exec_command", "glob", "grep", "interrupt_process",
            "question", "queue_create", "queue_info", "queue_listen", "queue_push", "queue_take", "read",
            "read_image", "request_write_permission", "run_agent_tasks", "set_checkpoint", "status", "wait", "wait_agent",
            "wait_process", "web_fetch", "write_stdin", "write",
        };
