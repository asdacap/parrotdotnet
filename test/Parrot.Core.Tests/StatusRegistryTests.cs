...
internal sealed class StatusRegistryTests
{
    ...
    {
        var query = new StatusQuery("session", string.Empty, string.Empty, "plan", "openai/gpt/high", null);
        var calls = 0;
    ...
    public async Task Context_reports_available_and_unavailable_windows_exactly(long estimatedTokens, int contextLimit, int usage, bool available, string formattedTokens, string formattedLimit)
    {
        ...
        var observation = await provider.Observe(new StatusQuery("session", string.Empty, string.Empty, "build", "provider/model", null), CancellationToken.None);
    ...
    public async Task Context_reports_exact_override_without_replacing_model_window(long? triggerTokens, string expectedTrigger)
    {
        ...
        var observation = await provider.Observe(
            new StatusQuery("session", string.Empty, string.Empty, "build", "provider/model", null),
    ...
    public async Task Selection_supports_structured_overrides(string parentSessionId, string parentSessionName, string expected)
    {
        ...
        var observation = await provider.Observe(new StatusQuery("child", parentSessionId, parentSessionName, "build", "{{ model }}", null), CancellationToken.None);
    ...
    public async Task Additional_context_provider_preserves_order_and_runtime_exclusions()
    {
        ...
        var query = new StatusQuery("session", string.Empty, string.Empty, "build", "provider/model", null);
    ...
    public async Task Selection_reports_a_complete_canonical_requested_selector()
    {
        ...
        var observation = await provider.Observe(
            new StatusQuery("session", string.Empty, string.Empty, "build", "provider/model/medium", null),
            CancellationToken.None);
    ...
    public async Task Selection_reports_parent_context_when_present(string parentSessionId, string parentSessionName)
    {
        IStatusProvider provider = new SelectionStatusProvider(TestModels.PromptTemplates);
        var observation = await provider.Observe(
            new StatusQuery("child", parentSessionId, parentSessionName, "build", "low_llm", null),
            CancellationToken.None);
    ...
    public async Task Runtime_tree_nests_queues_processes_and_active_agents(CancellationToken cancellationToken)
    {
        ...
        _ = child.GetService<IProcessOwner>().StartPipe("fetch", "sleep 30", "call", ProcessEnvironmentOverrides.Empty, child.Session, SecurityProfile.Compose(readOnly: false, [], [], []));
        _ = root.GetService<IProcessOwner>().StartPipe("build", "sleep 30", "call", ProcessEnvironmentOverrides.Empty, root.Session, SecurityProfile.Compose(readOnly: false, [], [], []));
        IStatusProvider provider = new RuntimeTreeStatusProvider(fixture.Registry, TestModels.PromptTemplates);

        var observation = await provider.Observe(
            new StatusQuery("root", string.Empty, string.Empty, "build", "provider/model", null),
            cancellationToken);
