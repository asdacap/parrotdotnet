...
internal sealed class WebServerTests
{
    ...
    {
        await using var server = await WebServer.Start(new TestService(), new TestWebService(), 0, cancellationToken);
        ...
        var modes = await ListModes(address, cancellationToken);
    ...
    private static async Task<ListModesResponse> ListModes(Uri address, CancellationToken cancellationToken)
    {
        ...
        return await client.ListModesAsync(new ListModesRequest(), cancellationToken: cancellationToken);
    }
