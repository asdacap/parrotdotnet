...
internal sealed class SessionsCommandTests : IDisposable
{
    ...
    [Test]
    public async Task Lists_named_and_legacy_sessions_in_creation_order(CancellationToken cancellationToken)
    {
    ...
    private static SessionMeta Meta(string id, string name, string model, string createdAt) =>
        new()
        {
            Id = id,
