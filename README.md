...
Parent addressing is disabled by default: the advertised `agent_send`
description drops the parent wording and a send that resolves to the parent
returns a tool error, while descendant sends and the automatic completion
delivery are unaffected. Setting `agent_send_to_parent: true` in the user config
enables parent addressing.
