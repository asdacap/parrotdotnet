...
internal sealed class ImageArtifactRepository(ImageArtifactStore store, IEventRepository events) : IImageArtifactRepository
{
    public Task<ImageArtifactMetadata> Persist(
        ...
        PersistDeclared(source, uploadId, displayName, origin, string.Empty, -1, cancellationToken);

    public async Task<ImageArtifactMetadata> PersistDeclared(
        Stream source,
        string uploadId,
        string displayName,
        string origin,
