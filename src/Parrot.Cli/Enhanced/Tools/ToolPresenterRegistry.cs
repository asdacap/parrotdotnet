...
internal sealed class ToolPresenterRegistry
{
    ...
    public ILiveBufferItem PresentLive(ToolCallPresentation call, int frame)
    {
        ...
        try
        {
            return presenter.PresentLive(call, frame);
        }
        catch when (!ReferenceEquals(presenter, _fallback))
        {
            return _fallback.PresentLive(call, frame);
        }
    }
    ...
    public IScrollbackItem? PresentTerminal(ToolCallPresentation call, ToolTerminalPresentation terminal)
    {
        ...
        try
        {
            return presenter.PresentTerminal(call, terminal);
        }
        catch when (!ReferenceEquals(presenter, _fallback))
        {
            return _fallback.PresentTerminal(call, terminal);
        }
    }
