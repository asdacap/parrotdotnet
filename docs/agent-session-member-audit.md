...
`Identity` and `Activity` are retained because other sessions/scopes need identity or activity observations. Child-tree ownership is deliberately not a session member: scope-sensitive callers receive the exact owning `IAgentSessionScope`, whose `ChildRegistry` and `ChildQuestions` form the relationship boundary. Own-session tool factories receive scoped dependencies directly and do not traverse session members as dependency proxies.
